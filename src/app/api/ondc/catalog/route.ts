 ...
 */
function priceOf(item: CatalogItem): string | null {
  const value = item.salePrice ?? getListingPrice(item);
  if (value === null || value === undefined || !Number.isFinite(value)) return null;
  return value.toFixed(2);
}

function absoluteImages(images: string[], origin: string): string[] {
  return images
...
export async function GET(req: Request) {
  try {
    const origin = process.env.ONDC_BPP_URI_ORIGIN || new URL(req.url).origin;
    ...
    const rows = (await prisma.craftItem.findMany({
      where: { isListedOnMarketplace: true },
