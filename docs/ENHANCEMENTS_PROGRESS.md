...
§13.1 opens by saying the code and the intended narrative disagree. **They do not disagree with each other — the code and the copy agree.** `src/lib/escrow.ts` documents `CREATOR_RATE = 0.05` as taken from the platform-side remainder, never the artisan's share; `/creators` says "The 5% comes out of Karigari's own share, never the artisan's"; the marketplace and `creator_commission_note` both say "The artisan's share is unchanged". Skipping phase 13 therefore leaves a **self-consistent product that is more generous to the artisan than the phase would have made it.** The only thing that can now disagree with the build is a pitch that describes the commission as artisan-funded and opt-in. That is a narrative to correct, not code.

### Appendix A — reconciled against the schema AND the live database
Every model and relation phases 1–12 were allowed to add is present in `prisma/schema.prisma` and as a table in Postgres: `OfflineSale`, `MarketplaceSearch`, `CreditProfileShare`, `LearningProgress`, `SupplyReminderState`, `ArtisanBadge`, `DesignConcept`, `MotifRegistration`, `MotifLicence`, `MotifPayoutShare`, `ScrapLot`, `ScrapPool`, `ScrapEnquiry`, `ScrapPayoutShare`. None of phase 13's three `CraftItem` columns exists in either — the schema is not ahead of the code. `SOLD_OFFLINE` is handled in `SOLD_STATUSES` and `statusBadge()`, and is deliberately still absent from `STOREFRONT_SOLD_STATUSES`, which counts storefront purchases while offline sales are read from their own table.

### Appendix B — now executable: `npm run verify:guardrails`
`scripts/verify-guardrails.mjs` (plain Node, no test runner, the same convention as `verify-schemes.ts`) asserts the machine-decidable half of the checklist across all 174 source files this programme touched. **57 passed, 0 failed.** It covers: no new npm dependency (`package.json` deps byte-identical to `e12da52`); no `alert`/`confirm`/`prompt`; all four dictionaries carrying the same 2,249 keys; no string in any language claiming a GI, legal protection, a credit rating, a blockchain or AI authentication; the four income streams staying separate; payout mode derived from the payout result rather than asserted; `unoptimized` on every data-URL `<Image>`; the `@@unique` and timestamp guards on every money step; public routes built from allow-lists; and `logCraftItemEvent()` on every `CraftItem` mutation.

The script says in its own header which checklist items it does **not** cover — empty states, medians, aria, hydration-safe dates, 360 px, console cleanliness — because those were verified in the browser phase by phase and a regular expression cannot honestly re-assert them.

### One thing the audit found and this commit fixes
Two dictionary strings claimed a blockchain this project does not have: `blockchain_patch_id` ("Blockchain Patch ID") and `capture_subtitle` ("…register your next piece **on the blockchain** in under 2 minutes"), in every language that defined them. There is no chain anywhere in the codebase. Both keys turned out to be **dead** — nothing renders them, so no user ever saw the claim — but it is exactly the claim Appendix B forbids, and a dead claim is one `t()` call away from being a live one. Rewritten rather than deleted, so the keys survive for whoever intended them: "Patch ID", and "Photograph and register your next piece in under 2 minutes."

Three further hits were **my own audit being too broad**, and the rule was narrowed rather than the copy changed: "GI tag news" on the news page, "GI tag needed" in a buyer's placeholder, and "your GI tag" (the one an artisan declared on their own profile) all name a GI without this app claiming to confer one.

### Known follow-ups carried out of the programme
- Every seeded artisan is alone in their own cluster. Phases 8, 9, 11 and 12 each proved their cluster behaviour by temporarily joining artisans and restoring them afterwards; nothing in the seed exercises a real multi-member cluster.
- `/api/admin/facilitator-queue` selects every `CraftItem` column, including base64 images, for all 105 rows. It took over two minutes during phase 12's verification and saturated the connection pool badly enough to queue an unrelated request behind it. It is the slowest thing in the app.
- The facilitator dashboard reports a 377 px scroll width at a 360 px viewport on all five of its tabs, including the four that predate this programme.
- `LanguageSwitcher` renders a 36 px tap target on every public page.
- `blockchain_patch_id` still carries "blockchain" in its **key name**, though its value no longer does. Renaming a dead key across four dictionaries was not worth the churn.
- Appendix D's demo order is still a good smoke test, with one correction: its step 10 offers "the artisan-funded opt-in commission with its real rupee comparison" as a Q&A backstop. That feature does not exist. The honest version of that answer is the one the code and the copy already give — the creator's 5% comes out of Karigari's share and the artisan's earnings are untouched by promotion.
