...
| 9 | Recognition & Anonymous Cluster Benchmarks | DONE | (this commit) | 2026-09-18 | Badges awarded only by `evaluateBadges()` from the artisan's own rows — never by an admin, never for signing in — with the frozen figures that earned each one, and a k-anonymous cluster comparison that shows medians only when at least 5 other artisans are active and otherwise returns no figure at all, in the UI or the JSON. 42 unit checks, 45 live API checks, browser checks in four languages at 360 px. |
